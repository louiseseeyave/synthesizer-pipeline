...
def get_lum_weighted_metallicity(_gal, grid, filters):
...
def get_young_mass_weighted_metallicity(_gal):

    """
    Get initial mass-weighted metallicity of young (<10 Myr) stars.
    """
    
    ages = (_gal.stars.ages).to('Myr')
    ok = ages < 10
    initial_masses = _gal.stars.initial_masses[ok]
    metallicity = _gal.stars.metallicities[ok]

    weighted_metallicity = \
        np.dot(metallicity, initial_masses)/np.sum(initial_masses)
...
def save_filters(fname):
    
    """
    This has to be done separately, doesn't work with MPI it seems.
    """
